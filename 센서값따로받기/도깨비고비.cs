...
namespace 센서값따로받기
{
    public partial class 도깨비고비 : Form
    {
        ...
        private void showValue(Int32[] s)
        {
           double moisture;
            moisture = (s[0] / 1023) * 100;
        
            sensorData data = new sensorData(
                DateTime.Now.ToShortDateString(),
                DateTime.Now.ToString("HH:mm:ss"), s[0]);

                myData.Add(data);
            string item = DateTime.Now.ToString() + '\t' + s[0];
            //listBox1.Items.Add(item);
                listBox1.SelectedIndex = listBox1.Items.Count - 1;

            chart1.Series["s[0]"].Points.Add(s[0]);
            chart1.ChartAreas["draw"].AxisX.Minimum = 0;
